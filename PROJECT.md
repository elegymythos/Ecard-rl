...
本项目用自博弈 PPO 模拟「皇帝牌」博弈中的有限理性学习动力学。当前核心认知：

1. **理性均衡**：`solver.py` 验证 V₄=-0.2、首轮 p=q=0.2、胜率 0.8。
2. **主观 Nash 不变性**：在当前效用层下，λ/α/β/ref 常数权重不改变均衡策略；心理参数不产生“新的均衡”。
3. **PPO 落点是 min-ent=0.5 下的弱平稳中心**：run06 显示 p≈0.20、q≈0.25；但 run11_minent 显示该落点强烈依赖 min-ent 强度。
4. **λ↑→p↓ 是梯度尺度效应**：run18 在 adv-norm 开启后该效应消失。
5. **q-p≈+0.05 机制未完全解决**：不是曲率、收益量级、初始化、共享躯干；更新相位是新的候选机制。

**当前证据边界**：无人类数据；“现实均衡”仍为模拟侧假设；所有落点结论限定在 min-ent=0.5、5 seeds、400k 步。
